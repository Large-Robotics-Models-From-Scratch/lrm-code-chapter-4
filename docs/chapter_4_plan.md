...
Per `book_learnings.md` §1.1, every body chapter restores state from the prior chapter before adding new state. Section 4.2 opens with two short code snippets: (a) loading the trained Chapter 3 backbone via `torch.load("ch3_backbone.pt")` and (b) calling `backbone.forward(image, instruction)` to confirm we still get a `[B, d]` tensor. No new content — just a sanity-check ritual so the reader can re-anchor.
...
* Three-line code block re-instantiating the trained VLA backbone, confirming output shape `[B, d_embed=768]`.
* One paragraph mapping the chapter's task: bridge this `[B, 768]` tensor to a per-joint action prediction.
...
| Autoregressive | reserved-vocab action tokens decoded `H · D` steps through the causal fusion transformer | moderate (`H · D` cached steps, once per chunk) | yes | RT-2, OpenVLA, MotionLM | **shipped — §4.5.4** |
...
* Three vertical panels with the *same* incoming context from the Chapter 3 fusion transformer splitting into three head designs.
  ...
  * **(b) Autoregressive (shipped).** Reserved-vocab action-token embeddings appended to the fused sequence; the causal fusion transformer decodes `H·D` tokens one at a time, each attending to the perception prefix and all earlier action tokens.
...
* **Listing 4.7 — `ParallelActionHead`.** ~25 lines. A two-layer MLP (GELU) that reads the fusion transformer's final state token and projects `d_embed → action_dim · chunk_len · n_bins`, reshapes to `[batch, H, D, B]`, exposes `forward`, `loss`, and `predict`.
...
* "Let's run it." Instantiate on a `[4, 512]` fake backbone summary, forward → assert shape `[4, 16, 6, 256]`. Compute loss against random integer targets, assert near `log 256 ≈ 5.54` (untrained = uniform).
...
* **The mechanism, adapted to our stack.** Unlike OpenVLA (a single LLM), our Chapter 3 backbone is SigLIP + SmolLM feeding a separate 512-dim causal fusion transformer. So "reuse the LM head" becomes: reserve the last 256 ids of SmolLM's vocab as `<act_0..act_255>` (the reservation lives in Ch4's tokenizer, §4.4.5, not Ch3 source), embed action tokens, append them to the fused sequence `[img | lang | state | a_1 … a_{H·D}]`, run the existing causal fusion transformer, and read each action position's logits through a 512→256 action head. One shared cross-entropy objective over the action positions.
...
* A parallel linear head exposes the cost of per-dimension independence: correct marginals, incoherent joint samples. The autoregressive head — reserved-vocab action tokens decoded left-to-right through the Chapter 3 fusion transformer with a causal mask and a KV-cache — fixes it, and trained with bf16 AdamW for ~20,000 steps on a consumer GPU produces a working policy on the SO-101 pick-and-place task.
...
| Listing 4.10 | `AutoregressiveActionHead` (teacher-forced) | 4.5.5 | The shipped head — append-and-decode through the fusion transformer |
...
* **From Chapter 3 →.** Reuse the backbone built in Ch 3. Its fusion transformer is causal end-to-end; the autoregressive head makes that mask the *correct* design (action tokens attend causally to the perception prefix and to earlier action tokens), so no Chapter 3 rework is needed. The earlier suggestion to relax Ch 3 to a bidirectional/prefix-LM mask is withdrawn — it was premised on the parallel head. Chapter 4 appends reserved-vocab action tokens to the fused sequence and decodes through that same transformer.
